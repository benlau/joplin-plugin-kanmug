...
import {
  RecentKanbansPanel,
  useRecentKanbansPanelHandle,
} from "./RecentKanbansPanel";
...
function Content(props: { board?: BoardState }) {
  ...
  const handleRemoveKanban = React.useCallback(
    (noteId: string) => {
  ...
  const cont = board ? (
    <Container>
      <Header>
        ...
        <BoardTitle onClick={() => send({ type: "openKanbanConfigNote" })}>
          {board.name}
        </BoardTitle>
        ...
        <IconCont onClick={handleRefresh}>
      ...
      <RecentKanbansPanel {...recentKanbanHandle} />
    </Container>
  ) : (
    <h1>
      <LoadingCont>
