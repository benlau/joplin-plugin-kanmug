...
export default class Board {
  ...
  getBoardState(allNotes: NoteData[]): BoardState {
    ...
    if (this.isValid) {
      ...
      state.columns = sortedColumns.map((column) => {
        const link = this.parsedConfig?.columns.find(
    ...
    switch (action.type) {
      ...
      case "moveNote":
        return this.moveNote(
          ...
          action.payload.newColumnName,
          action.payload.oldColumnName,
          action.payload.newIndex,
          boardState,
        );
      case "insertNoteToColumn":
        return this.insertNoteToColumn(
          action.payload.noteId,
      ...
      case "moveNoteToTop":
        return this.moveNoteToTop(
          action.payload.noteId,
          action.payload.columnName,
          boardState,
        );
      case "moveNoteToBottom":
        return this.moveNoteToBottom(
          action.payload.noteId,
          action.payload.columnName,
          boardState,
        );
  ...
  mergeCachedNotes(notes: NoteData[]): NoteData[] {
    const newNotes = notes.filter(
      (note) =>
        !this.cachedNotes.find((cachedNote) => cachedNote.id === note.id),
    );
    return [...this.cachedNotes, ...newNotes];
  }

  executeUpdateQuery(query: UpdateQuery) {
    this.cachedNotes = this.cachedNotes.map((note) => {
